...
suspend fun createSheet(context: Context, stairs: Int, sections: Int) {
    val workbook = Workbook()
    val worksheet = workbook.worksheets.get(0)
    val start:Char = 'B'
    val start_ascii = start.code
    val ascii_end = start_ascii + sections
    val end = ascii_end.toChar()
    val sheet = mutableListOf<Any>()
    for(i in 0..stairs){
    ...
    var x = "${start}1:${end}${stairs + 1}"
//    worksheet.getRange("${start}1:${end}${stairs + 1}").value = sheet
...
//    worksheet.getRange("E6:G6").getBorders().get(BordersIndex.EdgeBottom)
//        .setLineStyle(BorderLineStyle.Medium)
//    worksheet.getRange("E6:G6").getBorders().get(BordersIndex.EdgeBottom)
//        .setColor(Color.FromArgb(32, 61, 64))
//    worksheet.getRange("E7:G7").getBorders().get(BordersIndex.EdgeBottom)
//        .setLineStyle(BorderLineStyle.Medium)
//    worksheet.getRange("E7:G7").getBorders().get(BordersIndex.EdgeBottom)
//        .setColor(Color.FromArgb(32, 61, 64))
...
//    worksheet.getRange("E3:F3").getBorders().setColor(Color.FromArgb(32, 61, 64))
    ...
    try {
        var fos: FileOutputStream = context.openFileOutput("SimpleBudget.xlsx", Context.MODE_PRIVATE)
        workbook.save(fos)
